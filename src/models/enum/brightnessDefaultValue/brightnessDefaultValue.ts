...
export function setBrightnessDefaultValue() {
    ...
    if(store.$state.color.result == store.$state.color.nodes[5].result || store.$state.color.result == store.$state.color.nodes[6].result ) {
        ...
        store.$state.brightness.nodes[2].disabled = true;
        store.$state.brightness.nodes[2].result = OffNodesEnum.result;
        store.$state.brightness.nodes[2].selected = OffNodesEnum.selected;
    }
};